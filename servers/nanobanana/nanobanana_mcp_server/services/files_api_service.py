...
class FilesAPIService:
    ...
    def create_file_data_part(self, file_id: str) -> gx.Part:
        """
        Create file_data Part for Gemini API calls.

        Ensures file is available and creates a proper gx.Part object
        compatible with google-genai SDK v1.63+.

        Args:
            file_id: Files API file ID

        Returns:
            gx.Part with file_data for Gemini API
        ...
        try:
            ...
            # Build a proper SDK Part object (compatible with google-genai v1.63+)
            file_data_part = gx.Part.from_uri(file_uri=file_uri, mime_type=mime_type)
